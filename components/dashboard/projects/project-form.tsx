...
type FormState = ProjectFormInitial
...
export function ProjectForm({
  ...
  const validate = () => {
    ...
    if (typeof form.featured !== "boolean")
      nextErrors.featured = "Choose featured state."
    if (!userId) nextErrors.global = "No author profile was found for this project."
    if (mode === "edit" && (projectId === undefined || projectId < 1)) {
      nextErrors.global = "Missing project identifier."
    }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const url =
      mode === "create"
        ? "/api/projects"
        : `/api/projects/${projectId as number}`
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between rounded-xl border border-border bg-muted/40 px-3 py-2">
          <div className="flex flex-col">
            ...
            <span className="text-xs text-muted-foreground">
              Highlight this project on your homepage or listings.
            </span>
          </div>
          <button
            type="button"
            ...
            className={cn(
              "relative inline-flex h-6 w-11 items-center rounded-full transition-colors",
              form.featured ? "bg-primary" : "bg-muted-foreground/40",
            )}
          >
            <span
              className={cn(
...
export function NewProjectForm({
  userId,
  techStackOptions,
}: Pick<ProjectFormProps, "userId" | "techStackOptions">) {
  return (
    <ProjectForm mode="create" userId={userId} techStackOptions={techStackOptions} />
  )
}
